...
import com.example.authapp.application.admin.usecase.AdminConsoleUseCase;
...
class AdminConsoleUseCaseTest {
    ...
    private AdminConsoleUseCase service() {
        return new AdminConsoleUseCase(
                userRepository,
    ...
    void usersCanBeFilteredByHrMasterAndDirectSignupFields() {
        AdminConsoleUseCase service = service();
    ...
    void disablingUserRevokesAllUserSessionsWithReason() {
        ...
        verify(refreshTokenRepository).revokeActiveByUsername(eq("lee.user"), eq("ACCOUNT_DISABLED"), eq("UNKNOWN"), any());
    }

    @Test
    void revokeSessionRecordsExplicitRevokeSessionAuditAction() {
        AdminConsoleUseCase service = service();
        ...
        verify(adminActionLogService).record(argThat(request ->
                request.actionType() == AdminActionType.REVOKE_SESSION
                        && "USER".equals(request.targetType())
                        && "lee.user".equals(request.targetUsername())
                        && request.afterValue().contains("\"sessionId\":100")));
    }
}
