...
public class RoleAssignmentService {
    ...
    public void assignUserRole(String username, AdminRoleAssignmentRequest request) {
        ...
        if (!user.getRoles().contains(role)) {
            String before = userRolesSnapshot(user);
            user.addRole(role);
            String after = userRolesSnapshot(user);
            saveHistory("USER", username, defaultText(user.getNickname(), username), role, "ASSIGN", request);
            recordRoleAssignmentAction("USER", username, defaultText(user.getNickname(), username), role, AdminActionType.ASSIGN_ROLE, request.reason(), before, after);
    ...
    public void revokeUserRole(String username, Long roleId, String reason) {
        ...
        if (user.getRoles().contains(role)) {
            ...
            recordRoleAssignmentAction("USER", username, defaultText(user.getNickname(), username), role, AdminActionType.REMOVE_ROLE, reason, before, after);
            rbacAuthorizationService.invalidateUserPermissionCache(username);
        }
    }
    ...
    public void assignGroupRole(Long groupId, AdminRoleAssignmentRequest request) {
        ...
        if (!group.getRoles().contains(role)) {
            String before = groupRolesSnapshot(group);
            group.addRole(role);
            String after = groupRolesSnapshot(group);
            saveHistory("GROUP", String.valueOf(groupId), group.getName(), role, "ASSIGN", request);
            recordRoleAssignmentAction("GROUP", String.valueOf(groupId), group.getName(), role, AdminActionType.ASSIGN_ROLE, request.reason(), before, after);
    ...
    public void revokeGroupRole(Long groupId, Long roleId, String reason) {
        ...
        if (group.getRoles().contains(role)) {
            ...
            recordRoleAssignmentAction("GROUP", String.valueOf(groupId), group.getName(), role, AdminActionType.REMOVE_ROLE, reason, before, after);
            rbacAuthorizationService.invalidateAllUserPermissionCache();
        }
    }
    ...
    private String roleMetadata(RoleEntity role) {
        String permissionCodes = role.getPermissions().stream()
                .map(permission -> "\"" + safe(permission.getCode()) + "\"")
    ...
    private String userRolesSnapshot(UserEntity user) {
        return "{\"roles\":[" + roleNamesJson(user.getRoles()) + "]}";
    }

    private String groupRolesSnapshot(GroupEntity group) {
        return "{\"roles\":[" + roleNamesJson(group.getRoles()) + "]}";
    }

    private String roleNamesJson(java.util.Collection<RoleEntity> roles) {
        return roles.stream()
                .map(RoleEntity::getName)
                .sorted(String.CASE_INSENSITIVE_ORDER)
                .map(roleName -> "\"" + safe(roleName) + "\"")
                .collect(Collectors.joining(","));
    }
